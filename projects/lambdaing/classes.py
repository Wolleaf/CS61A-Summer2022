...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        for i in range(1, -1, -1):
            player.hand.append(player.deck.cards[i])
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if not player.hand:
            return
        best_card = None
        highest_power = player.hand[0].power(opponent_card)
        for card in player.hand:
            if card.power(opponent_card) > highest_power:
                highest_power = card.power(opponent_card)
                best_card = card
