...
def oink_points(player_score, opponent_score):
    ...
    scores = tens * 2 - ones
    return scores if scores > 1 else 1
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            dice_score = take_turn(
                strategy0(score0, score1), score0, score1, dice, goal)
            score0 = score0 + dice_score
            additional_score = pigs_on_prime(score0, score1)
            score0 = score0 + additional_score
        else:
            dice_score = take_turn(
                strategy1(score1, score0), score1, score0, dice, goal)
            score1 = score1 + dice_score
            additional_score = pigs_on_prime(score1, score0)
            score1 = score1 + additional_score
        who = next_player(who)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader == last_leader or leader == None:
        ...
        message = "Player " + \
            str(leader) + " takes the lead by " + str(abs(score0 - score1))
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num_rolls = 0
    top_scores = 0
    average_scores = make_averaged(roll_dice, total_samples)
    for i in range(1, 11):
        now_scores = average_scores(i, dice)
        if now_scores > top_scores:
            top_scores = now_scores
            num_rolls = i
    return num_rolls
    # END PROBLEM 9
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if o_points >= threshold:
        return 0
    now_score = score + o_points
    addtional_points = pigs_on_prime(now_score, opponent_score)
    if addtional_points > 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
