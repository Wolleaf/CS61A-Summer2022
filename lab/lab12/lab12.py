...
def link_pop(lnk, index=-1):
    ...
    lst = lnk
    if index == -1:
        while lst.rest.rest is not Link.empty:
            lst = lst.rest
        removed = lst.rest.first
        lst.rest = Link.empty
    else:
        while index != 1:
            index -= 1
...
def add_trees(t1, t2):
    ...
    if t2 is None:
        return t1
    t1.label = t1.label + t2.label
    length_t1, length_t2 = len(t1.branches), len(t2.branches)
    if length_t1 >= length_t2:
        for i in range(0, length_t2):
            add_trees(t1.branches[i], t2.branches[i])
    elif length_t2 > length_t1:
        for i in range(0, length_t1):
            add_trees(t2.branches[i], t1.branches[i])
        t1.branches = t2.branches
    return t1
...
def address_oneline(text):
    ...
    cardinal_dir = r'([NSEW]\s)?'  # whitespace is important!
...
def deck(suits, ranks):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[a, b] for a in suits for b in ranks]
